...
/* Los datos de los 12 terrenos viven en js/data.js (objeto global LOTES),
   compartidos con la ficha de villa para que mapa y ficha nunca difieran. */
...
const MARKER_POS = {"1":[55.79,42.77],"2":[69.35,46.33],"3":[82.01,48.77],"4":[49.95,48.71],"5":[65.35,52.18],"6":[79.94,55.11],"7":[45.73,55.08],"8":[61.35,58.24],"9":[78.17,62.02],"10":[40.93,61.95],"11":[56.94,65.95],"12":[75.54,72.39],"B1":[41.01,40.77],"B2":[26.04,58.41]};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Describe each marker for screen readers (otherwise it's just a number).
    markers.forEach(m => {
        const t = LOTES[m.dataset.id];
        if (t) m.setAttribute('aria-label',
            `Terreno ${m.dataset.id}: ${t.sup} m², ${loteLinea(m.dataset.id)}, ${t.estado}`);
    ...
    blkChips.forEach(c => c.setAttribute('aria-pressed', c.classList.contains('active') ? 'true' : 'false'));
    blkChips.forEach(chip => {
        chip.addEventListener('click', () => {
            const filter = chip.dataset.filter;
            blkChips.forEach(c => { c.classList.remove('active'); c.setAttribute('aria-pressed', 'false'); });
    ...
    viewChips.forEach(v => v.setAttribute('aria-pressed', v.classList.contains('active') ? 'true' : 'false'));
    const modeMap = { 'Día': 'dia', 'Atardecer': 'sunset', 'Plano': 'plan' };
    viewChips.forEach(v => {
        v.addEventListener('click', () => {
            viewChips.forEach(x => { x.classList.remove('active'); x.setAttribute('aria-pressed', 'false'); });
